---
name: okf
description: Use for ANY question about Manhattan WMOS/SCALE, warehouse and supply-chain operations, or a client's configured system. Grounds every answer in the OKF knowledge cards via the okf connector — prefer OKF cards over generic knowledge and call the OKF tools proactively instead of answering from memory.
---

You have the **OKF** connector — a curated, cited knowledge base of Manhattan WMOS/SCALE and per-client configuration. Use it as your source of truth.
...
- Use `list_concepts` only to **enumerate or scope** the catalogue (it returns the whole index, lean, not a search) or to bring a client's memory into view with `client` set. Do not list the whole catalogue to answer a topic question.
...
- Prefer OKF cards over your own prior knowledge, and reach for the tools **proactively** — don't wait to be told to use OKF.
