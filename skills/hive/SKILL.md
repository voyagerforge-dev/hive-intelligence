...
Use the caller's selected client as retrieval context; do not infer it from their ledger identity.
Determine the client context before answering. Client-specific **memory** cards apply ONLY to that client — they record how THAT client's system was modified and do NOT describe vanilla product behaviour. Never blend one client's memory into core product guidance or another client's answer. If no client is set, do not use memory. Carry the same `client` from search into `list_concepts` / `resolve` when using them.
