...
3. [Stage 1: Document preparation (`hive-prep`)](#3-stage-1-document-preparation-hive-prep)
4. [Stage 2: Card creation (`hivegen`)](#4-stage-2-card-creation-hivegen)
5. [Stage 3: Serving (`hive-serve`)](#5-stage-3-serving-hive-serve)
...
flowchart LR
    ...
    subgraph G["② hivegen - card creation"]
        direction TB
        g1[taxonomy → distill]
    end
...
- **`hive-prep`** ([Stage 1](#3-stage-1-document-preparation-hive-prep)) - a human-gated,
...
- **`hivegen`** ([Stage 2](#4-stage-2-card-creation-hivegen)) - distills that atomic markdown
...
- **`hive-serve`** ([Stage 3](#5-stage-3-serving-hive-serve)) - serves
...
## 3. Stage 1: Document preparation (`hive-prep`)
...
| File | Job |
|---|---|
| `cli.py` | The `hiveprep` command group: `scan · dups · validate-plan · dedup-formats · normalize · route · transform · stamp · validate-atomic`. Builds the model clients from `.env` for `transform` only. |
...
**Deep dive:** [`tooling/hive-prep.md`](tooling/hive-prep.md) - every module's logic, inputs/outputs, and how it's invoked.
...
## 4. Stage 2: Card creation (`hivegen`)

**Goal:** distill the atomic markdown into **concept cards** - reusable, cross-linked knowledge,
one card per concept.

**Package:** `tooling/hive-gen/` (package `hivegen`). It is **concept-centric** and, like `hive-prep`,
...
| `memory.py` | The pure `record ⇄ memory-card` serialization seam - feeds the CLI, the Action, and hive-author. |
...
**Deep dive:** [`tooling/hive-gen.md`](tooling/hive-gen.md) - the 13 modules and 16 scripts (`scripts/`) in module-level detail.
...
|---|---|---|
| **1. Facet stamp** | `hive-gen/scripts/product_facet_apply.py <concepts> <atomic> <product> <platform>` (oSCI uses `osci_facet_apply.py`) | Stamps `product` + `platform` + `version` (union of the card's source-doc folder-years) onto the product's cards. |
| **1b. Version facet** | `hive-gen/scripts/version_apply.py <concepts>` | Deterministically derives `version` (release scope) from source-ref years - no LLM, no gate. Soft filter-with-fallback (no `resolve()` guard). |
| **1c. Regime facet** | `hive-gen/scripts/regime_apply.py <concepts>` (proposals from `regime_classify.py`) | Stamps `regime` (ops vs. traditional, within-product either-or) from a human-reviewed classification; drives the cross-regime `resolve()` expansion guard. |
| **2. Conformance pass** | `hive-gen/scripts/conformance_pass.py <concepts>` | Sets `resource` → served-card URI; adds the OKF-recommended `timestamp`; regenerates `## Related` (bundle-relative markdown links, from `related:`) and `# Citations` (from `sources:`) body sections. |
| **3. Index generation** | `hive-gen/scripts/index_generate.py <concepts>` | Writes the root `index.md` (`okf_version: "0.1"` frontmatter) + per-product `index.md` progressive-disclosure listings (concepts only - corrections excluded). |
...
**Isolation eval = the deploy gate.** Before deploy, `hive-serve/run_eval` over `data/<product>_product_qa.jsonl`
...
- **Authoring: from Claude first, GitHub optional.** The **primary entry is from Claude**, via the
  [`hive-author`](#the-memory-layer) server's `submit_correction` tool, which files an `okf-correction`
  ...
  `record ⇄ card` seam (`hivegen/corrections.py`). `CODEOWNERS` routes each product's corrections dir to
  the owner;
...
| Script / file | Job |
|---|---|
| `hive-gen/hivegen/corrections.py` | Pure `record_to_correction` / `correction_to_record` seam (shared by CLI + Action). |
| `hive-gen/scripts/new_correction.py` | CLI: scaffold a `status: draft` correction card under `concepts/<product>/corrections/`. |
...
**Promotion - personal → global, sanitize + approve.** `promote(memory_id)` (keyless; hive-serve only
...
Issue** - **primarily from Claude**, through the **`hive-author`** server's `submit_memory_promotion`
...
**`hive-author` - the write-only door.** So that hive-serve stays **keyless and read-only**, a separate,
...
| Script / file | Job |
|---|---|
| `hive-gen/hivegen/memory.py` | Pure `record_to_memory` / `memory_to_record` seam (shared by CLI + Action + hive-author). |
| `hive-gen/scripts/new_memory.py` | CLI: scaffold a `status: draft` memory card under `clients/<client>/memory/`. |
| `hive-gen/scripts/memory_from_issue.py` | Parse a Memory Issue-Form body → record → card (product allowlist + `client` `\A[a-z0-9-]+\Z` path guard). |
| `hive-gen/scripts/memory_lint.py` | Structural lint (bad client/product, dangling `related`, bad supersedes, status) + same-client conflict candidates. |
...
### The database-object tier (`hive-dbparse`)
...
`tooling/hive-dbparse/` (package `hivedbparse`); its parsing engine is **sqlglot** (a
...
**The hard verification gate is the point.** `hivedbparse/run.py` refuses to emit a partial corpus: the
...
| File | Job |
|---|---|
| `hivedbparse/model.py` | The dataclasses: `Table` (columns/PK/FK/index/seq/trigger), `Column`, `PlsqlObject` (signature + Oracle/DB2 bodies). |
| `hivedbparse/parse_tables.py` | `CREATE TABLE` → columns/types/PK/inline constraints; token-level fallback for Oracle storage clauses; db2→generic dialect map (**sqlglot has no `db2` dialect**). |
| `hivedbparse/parse_aux.py` | Overlays `COMMENT ON` (verbatim), foreign keys, indexes, sequences. |
| `hivedbparse/parse_plsql.py` | Package/proc/func/trigger/view boundary detection + **verbatim body** slice; standalone-`/` terminator; captures inline + `PLSQL_Objects/` units. |
| `hivedbparse/reconcile.py` | Oracle ⇄ DB2 match-by-name, type/body deltas, package spec+body merge. |
| `hivedbparse/emit.py` | One card (+ `manifest.jsonl` line) per object; deterministic `card_id`; safe-yaml frontmatter. |
| `hivedbparse/run.py` | The CLI + **hard verification gate** (count-match, unparsed→fail, dedup+`conflicts.log`); walks both dialect trees. |
...
### The issue-journal tier (`hive-zendesk`)
...
**Package:** `tooling/hive-serve/` (package `hiveserve`, CLI `hiveserve`).
...
Two transports, one codebase (`server.py`): `hiveserve serve --stdio` runs the MCP server over
stdio (the operator's Claude Code); `hiveserve serve --http` serves the REST routes **and** a
...
- **The database-object door**: `find_db_objects` - the only path to the on-demand
  [database-object tier](#the-database-object-tier-hive-dbparse). See below.
...
[database-object cards](#the-database-object-tier-hive-dbparse) are kept out of `list_concepts` (they'd
...
**Deep dive:** [`tooling/hive-serve.md`](tooling/hive-serve.md) - the core, both doors, the ledger, and the container in module-level detail.
...
flowchart LR
    d["FS-300 Replenishment.pdf<br/>(raw vendor spec)"]
      -->|hive-prep: curate→normalize→Docling| a["activity-tracking-fs300-replenishment.md<br/>(atomic, extracted_via: text)"]
      -->|hivegen: assign→distill→promote| c["concepts/activity-tracking-fs300-replenishment.md<br/>(concept card, related+sources)"]
      -->|hive-serve: resolve| q["Consultant asks Claude:<br/>'how does FS-300 replen trigger?'"]
...
2. **Create.** `hivegen` proposes an `activity-tracking-fs300-replenishment` concept, assigns this
...
**[tooling reference](tooling/README.md)**: [hive-prep](tooling/hive-prep.md) · [hive-gen](tooling/hive-gen.md) ·
[hive-serve](tooling/hive-serve.md) · [hive-dbparse](tooling/hive-dbparse.md) · [hive-author](tooling/hive-author.md) ·
[hive-zendesk](tooling/hive-zendesk.md).

### `tooling/hive-prep/hiveprep/` - Stage 1, doc prep
...
- see the [hive-prep file map](#hive-prep-file-map).

### `tooling/hive-gen/hivegen/` - Stage 2, card creation
...
[hivegen file map](#hivegen-file-map). Post-promote + authoring scripts live in `tooling/hive-gen/scripts/`
...
### `tooling/hive-dbparse/hivedbparse/` - the deterministic database-object parser
...
[The database-object tier](#the-database-object-tier-hive-dbparse).

### `tooling/hive-serve/hiveserve/` - Stage 3, serving
...
- see the [hive-serve file map](#hive-serve-file-map).

### `tooling/hive-author/hiveauthor/` - the write-only authoring door
...
`submit_correction`) · `server.py`. Keeps hive-serve keyless; see [The memory layer](#the-memory-layer).
...
| Thing | Where | Used by |
|---|---|---|
| **Bifrost** gateway (`VK_OKF`: minimax-m3 + deepseek-v4-flash) | Host-A `:4001` | hivegen (taxonomy/assign/distill) |
| **Docling** converter | Host-A GPU | hive-prep text tier |
| **Qwen3.6-27B** VLM (vLLM) | Host-D `llm-host.internal:8000` | hive-prep vision tier |
| **Cloudflare Access** (live gate; Authentik = self-hosted alt) | in front of hive-serve | serving identity/auth |
...
flowchart TB
    ...
    subgraph host-a["Host-A · hive-host.internal"]
        ...
        serve["hive-serve --http<br/>REST + /mcp"]
...
cd tooling/hive-prep && uv sync --extra dev
...
cd tooling/hive-gen && uv sync --extra dev
#   register the product's areas in hivegen/load.py AREAS (topic→area)
#   set ATOMIC_DIR + Bifrost VK_OKF in .env; run per area: SLICE_AREA=<area>
SLICE_AREA=<area> python -m hivegen.run   # gate 1: writes taxonomy.<area>.draft.yaml, stops
...
#   then: isolation eval must pass 0-bleed before deploy (hive-serve/run_eval)

# ── (one-off) database-object tier: deploy DDL → schema cards ───────────
cd tooling/hive-dbparse && uv sync --extra dev
...
| **hive-author** | The write-only MCP server (`issues:write` only) that files memory/correction authoring issues, keeping hive-serve keyless. |
