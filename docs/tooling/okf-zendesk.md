...
```bash
hivezendesk relink --client alpha --clients-dir ../../clients --concepts-dir ../../concepts \
...
Cards are served by `hive-serve` from a git checkout on Host-A, refreshed by the Windmill `cards_sync` job. **A card change reaches consultants only after it is merged to `main`** and that job runs. Note that `cards_sync` refreshes *data* only - anything touching `hiveserve` code needs an image rebuild.
