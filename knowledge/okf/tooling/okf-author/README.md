...
Spec-3 memory Action. It exists so **okf-serve stays keyless / read-only**: okf-author is the only
...
body (`_Submitted via okf-author by <email>_`).
...
- **Container** `okf-author`, bound to **`hive-host.internal:8016`** (VLAN60 LAN, same host model as okf-serve).
- **Image** built from this package via `deploy/Dockerfile` (the build context is the package dir).
- **Secret** decrypted from SOPS to **`/srv/okf-author/okf-author.env`** (mode 600, *outside* the git
  checkout) — never committed.
- **Compose project** is pinned to **`okf-author`** (`name: okf-author` in `deploy/compose.yml`).
  > ⚠️ **Gotcha:** both okf-serve and okf-author deploy dirs are named `deploy`, so without an explicit
...
(`/srv/okf-serve/repo/knowledge/okf/tooling/okf-author/deploy`):

```bash
# 1. Provision the token → SOPS (one-time; rotate before the PAT expires)
sops infra-repo/secrets/host-a/okf-author.enc.env      # GITHUB_TOKEN=github_pat_...

# 2. Decrypt → place on Host-A, outside the checkout (piped, never printed)
sops -d infra-repo/secrets/host-a/okf-author.enc.env \
  | ssh host-a 'sudo mkdir -p /srv/okf-author && sudo tee /srv/okf-author/okf-author.env >/dev/null \
                && sudo chmod 600 /srv/okf-author/okf-author.env'

# 3. Build + start (ensure the checkout is current first: wmill script run f/example/okf/cards_sync)
ssh host-a 'cd /srv/okf-serve/repo/knowledge/okf/tooling/okf-author/deploy \
...
If a stale `okf-author` container exists under the wrong project (the `deploy` collision), remove it by
name first — this leaves okf-serve untouched: `ssh host-a 'sudo docker rm -f okf-author'` then re-`up`.

## Public edge (CF Access) — for consultants

okf-author is LAN-only by default (reachable from Claude Code on the LAN). To let **consultants** submit
corrections / promote memory from **Claude Desktop / claude.ai**, front it with a Cloudflare Access app,
exactly like okf-serve (see `infra-repo/docs/runbooks/okf-mcp-cf-access-oauth.md`):

1. **CF Access app** (owner: Yash) — Zero Trust → Access → Applications → Add → Self-hosted. Name
   `okf-author`, hostname **`hive-author.example.com`** (no path), **Managed OAuth on**, team
...
2. **cloudflared** (owner: operator) — route `hive-author.example.com` **direct to `host-a:8016`**
   (bypass Caddy, like okf-serve), reload, validate the `/mcp` OAuth gate.
3. Consultants add **okf-author** as a second custom connector (alongside the read-only okf-serve one).

## Package layout

`okfauthor/config.py` · `identity.py` · `submissions.py` (issue-body builders + hard-separated `build_*`) ·
...
real `okf-gen` parsers to prove the issue bodies parse back losslessly).
