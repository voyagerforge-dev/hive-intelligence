# OKF tooling - okf-author (`okfauthor`)
...
### `submissions.py`
**Purpose.** Pure, side-effect-free builders that turn tool arguments into a GitHub issue payload (`title`, `body`, `labels`). This is where the two authoring paths are structurally kept apart and where round-trip compatibility with the okf-gen Issue-Form parsers is guaranteed.

**Key logic.**
- `_section(label, value)` emits `### <label>\n\n<value or "_No response_">\n`, exactly matching the GitHub Issue Form rendering the okf-gen parsers expect. `_prefix(submitted_by)` prepends `_Submitted via okf-author by <owner>._` when an owner is known.
...
**Invoked / deployed.** Called by `mcp_app.py`'s two tool functions. Exercised directly by `tests/test_submissions.py`, which parses the produced bodies back through the real okf-gen parsers.
...
**Key logic.**
- `build_mcp(settings, gh_factory)` constructs `FastMCP("okf-author", stateless_http=True, host=settings.host, port=settings.port)`. `gh_factory` is a zero-arg callable returning an `IssueClient`, so the real credential is injected from outside (and swapped for the fake in tests).
...
- The two tools mirror the Issue Forms via `submissions`, so a filed issue is directly consumable by the existing okf-gen memory/correction Actions after CODEOWNER approve-label.
...
**Dependencies.** internal: `okfauthor.submissions`, `okfauthor.identity`; external: `mcp.server.fastmcp` (`FastMCP`, `Context`).
...
### `server.py`
**Purpose.** The runnable entrypoint (console script `okfauthor`) that assembles the HTTP MCP server.
...
**Dependencies.** internal: `okfauthor.config`, `okfauthor.mcp_app`, `okfauthor.github_client`; external: `fastapi`, `uvicorn`, `mcp`.

**Invoked / deployed.** Container `CMD ["okfauthor"]` (the `project.scripts` entrypoint). `tests/test_server_http.py` covers the HTTP wiring.

## Deployment & runtime

Accurate to `deploy/compose.yml`, `deploy/Dockerfile`, and `README.md`:

- **Image / build.** `deploy/Dockerfile` is `python:3.12-slim`, `pip install uv` then `uv pip install --system -e .`, and bakes `ENV HOST=0.0.0.0`, `PORT=8016`, `IDENTITY_HEADER=Cf-Access-Authenticated-User-Email`; `EXPOSE 8016`; `CMD ["okfauthor"]`. The compose build context is `..` (the package dir), image tagged `okf-author:latest`.
- **Container on Host-A.** Service/container `okf-author`, `restart: unless-stopped`, published on `hive-host.internal:8016:8016` - VLAN60 LAN only (same host=VLAN60 model as okf-serve). `environment` sets `HOST=0.0.0.0` and `PORT=8016`. Healthcheck GETs `http://localhost:8016/healthz` every 30s.
- **Secret, outside the checkout.** `env_file: [/srv/okf-author/okf-author.env]` - the `GITHUB_TOKEN` (`issues:write` only) lives outside the git checkout, decrypted by the operator from SOPS `infra-repo/secrets/host-a/okf-author.enc.env` to `/srv/okf-author/okf-author.env` (mode 600, never committed). The token is the only secret; everything else is plain config.
- **Explicit compose project `name:` (anti-collision).** `deploy/compose.yml` pins `name: okf-author`. Both okf-serve and okf-author use a `deploy/` dir, so without this they collide on Compose's default project name - and a `docker compose down --remove-orphans` in one dir would remove the other's containers. If a stale container lands under the wrong project, the README fix is to remove it by name (`docker rm -f okf-author`) then re-`up`, leaving okf-serve untouched.
- **Public edge (CF Access), for consultants.** okf-author is LAN-only by default. To let consultants promote memory / file corrections from Claude Desktop / claude.ai, front it with its own Cloudflare Access app: a self-hosted app named `okf-author` on hostname `hive-author.example.com` (Managed OAuth on, team `homelab-gateway`, Allow policy scoped to consultant emails), with `cloudflared` routing `hive-author.example.com` direct to `host-a:8016` (bypassing Caddy, like okf-serve). Consultants then add **okf-author** as a second custom Claude connector alongside the read-only okf-serve one - two doors, one read (keyless okf-serve) and one write (`issues:write` okf-author). (CF Access app and cloudflared route are repo-external live infra - verify.)

## Tests

- **Posture.** Fakes-only, no live token or network. The GitHub client is injected: `tests/test_mcp_app.py` builds the app with `build_mcp(Settings(), lambda: FakeIssueClient())`, and `tests/test_github_client.py` exercises `FakeIssueClient` directly. Issue bodies are verified against the **real** okf-gen Issue-Form parsers: `tests/test_submissions.py` dynamically loads `okf-gen/scripts/memory_from_issue.py` and `correction_from_issue.py` and asserts the built body parses back losslessly (client/product/memory/related/citations for memory; corrects/correction/citations for correction), proving round-trip compatibility with the Actions. It also asserts the client-required guard raises `ValueError` and that `submitted_by` lands in the body. `test_mcp_app.py` asserts both tools register and that the memory/correction builders stay hard-separated (disjoint labels and body sections). `example-okfgen` is a dev-only dependency (relative source `../okf-gen`, editable) purely so these round-trip tests can import the parsers.
- **How to run.** `cd tooling/okf-author && uv run pytest -q` (per the doc header; the README uses `uv sync --extra dev && uv run pytest -q` to first pull the dev extras).
