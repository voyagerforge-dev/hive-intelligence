...
|---|---|---|---|---|
| `okf-prep` | raw vendor docs → atomic markdown | [Stage 1](../OKF-Pipeline.md#3-stage-1-document-preparation-okf-prep) | dev-box CLI (`uv`), orchestrated by `/wms-prep` | [okf-prep.md](okf-prep.md) |
| `okf-gen` | atomic markdown → concept cards, plus post-promote + corrections/memory authoring | [Stage 2](../OKF-Pipeline.md#4-stage-2-card-creation-okfgen) | dev-box CLI (`uv`) + a Windmill gate | [okf-gen.md](okf-gen.md) |
...
- **Sovereign + LLM-free at serve time.** Only Stages 1-2 (`okf-prep`, `okf-gen`) call models, on
  the firm's Bifrost gateway (`VK_OKF`); `okf-serve` and `okf-dbparse` call no model at all.
- **git is the system of record.** Cards are plain markdown files; the only mutable store is
  `okf-serve`'s SQLite objective/memory ledger.
- **Fakes-only tests.** Every package tests without live models or network:
  `cd tooling/<package> && uv sync --extra dev && uv run pytest -q`.
- **Two servers stay keyless-vs-keyed by design.** `okf-serve` holds no credential (read-only);
  `okf-author` holds the single `issues:write` GitHub token, in its own container.
...
- Operations: [Deploy okf-serve](../../tooling/okf-serve/deploy/README.md) and
