# OKF tooling - okf-dbparse (`okfdbparse`)
...
- **Frontmatter** is emitted via `yaml.safe_dump` (the same library okf-serve's loader parses back with `yaml.safe_load`), so a comment containing `:` or other YAML-special characters is always quoted correctly rather than hand-rolled. `platform` lists dialects in canonical order (`oracle`, `db2`).
...
**Key logic.**
- **Tree walk.** Per dialect it walks exactly two non-recursive globs: `Product/*.sql` (tables + inline PL/SQL) and `Product/PLSQL_Objects/*.sql` (PL/SQL units). `Path.glob("*.sql")` is non-recursive, so sibling dirs `Seed/`, `Archive/`, `Upgrade/`, `CreateSchema/` are skipped simply by never being walked. Files are sorted; `--limit-modules` caps files per dialect/kind for smoke runs. Each object's `module` is the filename stem and each object's `source_files` is the real repo-relative path(s).
...
- **CLI.** `main` uses argparse with `--src`, `--out`, and optional `--limit-modules`, then prints the report as JSON. The console entry point is `okfdbparse = okfdbparse.run:main`, and `python -m okfdbparse.run` works via `__main__`.
...
```
cd tooling/okf-dbparse
uv run python -m okfdbparse.run --src <ManhDBDeploy_root> --out ../../concepts/wms/db
```

(Note: `main` takes the flags `--src`/`--out` (and optional `--limit-modules`); the deploy paths are `<root>/{Oracle,DB2}/DBScripts/Product/*.sql` and `.../Product/PLSQL_Objects/*.sql`.) The only runtime dependency is `sqlglot` (plus `PyYAML` for frontmatter emission); no network, no model calls. It runs behind the **hard verification gate** so there is no partial corpus: any unparsed table statement or `CREATE OR REPLACE` unit fails the whole run, and every kind's emitted count must equal its parsed count. Same-dialect same-name duplicates are deduped (if identical) or logged to `conflicts.log` (if differing) but never fail the run. Re-run only when the source DDL changes.

Output: `tables/<NAME>.md` + `plsql/<NAME>.md` cards, `manifest.jsonl`, and (when duplicates exist) `conflicts.log`, all under `concepts/wms/db/`. These cards are served on demand by okf-serve behind `find_db_objects` and are deliberately kept out of `list_concepts`.
...
```
cd tooling/okf-dbparse && uv run pytest -q
```

Count: 81 test functions across `tests/test_parse_tables.py` (14), `test_parse_aux.py` (8), `test_parse_plsql.py` (19), `test_reconcile.py` (10), `test_emit.py` (16), and `test_run.py` (14).
