...
### `cli.py`
**Purpose.** The `okfprep` Click command group; each pipeline stage is one subcommand. Imports are lazy (inside each command body) so a subcommand only pays for the modules it needs.
...
**Invoked / deployed.** The `okfprep` console script; driven step-by-step by the `/wms-prep` orchestration command.
...
**Invoked / deployed.** `okfprep dups` (filtered to `.pdf/.docx/.pptx/.xlsx`).
...
**Invoked / deployed.** `okfprep validate-plan`, `okfprep dedup-formats`; `Plan`/`load_plan` are also consumed by `normalize`, `transform`, and `stamp`.
...
**Invoked / deployed.** `okfprep normalize`.
...
**Invoked / deployed.** `okfprep route` (precheck) and `okfprep transform` (conversion).
...
**Invoked / deployed.** `okfprep stamp`.
...
```
cd tooling/okf-prep && uv run pytest -q
```
