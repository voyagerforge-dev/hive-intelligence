...
`source_files` is unset (see `hivedbparse.emit._plsql_sources`).
...
from hivedbparse.reconcile import (
...
def _parse_dialect_tables(
    ...
    for sql_file, module, rel, is_seed in _dialect_table_files(src_root, dialect, limit_modules):
        ...
        with _capture_warnings("hivedbparse.parse_tables") as warnings:
        ...
        with _capture_warnings("hivedbparse.parse_plsql") as pl_warnings:
    ...
    if texts:
        ...
        #   hivedbparse.parse_aux      -- attachable DDL we couldn't parse. A real
        #                                silent drop (a lost index/sequence/FK/
        #                                comment) -> routed into the hard gate.
        #   hivedbparse.aux_unattached -- parsed fine, but its target isn't carded
...
def _parse_dialect_plsql(
    ...
    for sql_file in _sql_files(plsql_dir, limit_modules):
        ...
        with _capture_warnings("hivedbparse.parse_plsql") as warnings:
...
def run(src_root: Path, out_dir: Path, *, limit_modules: int | None = None) -> RunReport:
    ...
    with _capture_warnings("hivedbparse.aux_unattached") as seq_unattached:
    ...
    if report.unparsed:
        raise RunError(
            f"hivedbparse: {len(report.unparsed)} unparsed statement(s)/unit(s) "
            f"(never silently skipped): {report.unparsed}"
        )
    for kind, counts in report.counts.items():
        if counts["parsed"] != counts["emitted"]:
            raise RunError(
                f"hivedbparse: emitted count != parsed count for {kind!r}: {counts}"
            )
