...
# `package_name` click looks the metadata up under the module name and raises
# `RuntimeError: 'hiveprep' is not installed`, in a checkout and in the published
# wheel alike, because the distribution name is the same in both.
...
def dups(source, out):
    ...
    from hiveprep.dups import group_duplicates
...
def normalize(plan_path, work, jobs):
    ...
    from hiveprep.normalize import normalize_plan
    s = get_settings()
    plan = load_plan(Path(plan_path))
...
def validate_atomic_cmd(atomic_dir, relations_out):
    ...
    from hiveprep.validate import validate_atomic_dir, write_relations
