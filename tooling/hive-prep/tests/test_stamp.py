...
def test_stamp_missing_platform_adds_it(tmp_path):
    ...
    changed = stamp_file(f, {"platform": "PLATFORM"}, only_missing=True)
...
def test_stamp_only_missing_preserves_existing_product(tmp_path):
    ...
    f = tmp_path / "doc.md"
    f.write_text(MISSING_PLATFORM_FM)
    changed = stamp_file(f, {"platform": "PLATFORM", "product": "OVERRIDDEN"}, only_missing=True)
    assert "platform" in changed
    assert "product" not in changed
    text = f.read_text()
    assert "product: WIDGETS" in text
...
def test_stamp_overwrite_replaces_existing_field(tmp_path):
    ...
    assert "platform: PLATFORM" not in text
...
def test_stamp_no_change_when_all_fields_present(tmp_path):
    ...
    f = tmp_path / "doc.md"
    f.write_text(FULL_FM)
    changed = stamp_file(f, {"platform": "PLATFORM", "product": "WIDGETS", "version": "2024"}, only_missing=True)
    assert changed == []
...
def test_stamp_dir_skips_already_complete(tmp_path):
    ...
    result = stamp_dir(tmp_path, {"platform": "PLATFORM"}, only_missing=True)
    assert result == {}
...
def test_stamp_from_plan_fills_invariant_fields(tmp_path):
    ...
    plan = Plan(scope="", corpus_root="", subtree="", include=[
        {"path": "a/foo.docx", "platform": "PLATFORM", "product": "WIDGETS",
    ...
    assert "platform: PLATFORM" in text and "version: \"2024\"" in text
...
def test_stamp_from_plan_matches_passthrough_ext_suffixed_slug(tmp_path):
    ...
    (atomic / "widgets-x-appointment-2017-xsd.md").write_text(
        '---\ntitle: t\nslug: widgets-x-appointment-2017-xsd\nsource_doc: "Appointment_2017.xsd"\nstatus: active\n---\nbody\n')
    plan = Plan(scope="WIDGETS", corpus_root="/r", subtree=".", include=[
        {"path": "x/Appointment_2017.xsd", "platform": "PLATFORM", "product": "WIDGETS",
         "version": "2017", "doc_type": "technical-spec", "topic": "interfaces"}],
        exclude=[], dedup_groups=[], supersedes=[])
    stamp_from_plan(atomic, plan, only_missing=True)
    txt = (atomic / "widgets-x-appointment-2017-xsd.md").read_text()
...
def test_stamp_from_plan_matches_by_slug_not_basename(tmp_path):
    ...
    (atomic / "widgets-bench-b-overview.md").write_text(
        '---\ntitle: Overview\nslug: widgets-bench-b-overview\nsource_doc: "Overview.docx"\nstatus: active\n---\nbody\n')
    plan = Plan(scope="WIDGETS", corpus_root="/root", subtree=".", include=[
        {"path": "bench/a/Overview.docx", "platform": "PLATFORM", "product": "WIDGETS",
         "version": "2018", "doc_type": "documentation", "topic": "alpha"},
        {"path": "bench/b/Overview.docx", "platform": "PLATFORM", "product": "WIDGETS",
         "version": "2020", "doc_type": "documentation", "topic": "bravo"}],
        exclude=[], dedup_groups=[], supersedes=[])
    stamp_from_plan(atomic, plan, only_missing=True)
    a = (atomic / "widgets-bench-a-overview.md").read_text()
    b = (atomic / "widgets-bench-b-overview.md").read_text()
