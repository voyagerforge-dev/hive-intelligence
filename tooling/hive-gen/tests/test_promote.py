...
CARD_OK = ("---\ntype: concept\ntitle: Wave\ndescription: d\ntags: [w]\nresource: bench\n"
           "sources: []\nrelated: [gadgets/replenishment]\ndistilled_at: 2026-06-29\nstatus: approved\n---\n\nBody\n")
CARD_DRAFT = CARD_OK.replace("status: approved", "status: draft")
CARD_BADLINK = CARD_OK.replace("related: [gadgets/replenishment]", "related: [does-not-exist]")
CARD_NOREL = CARD_OK.replace("related: [gadgets/replenishment]", "related: []")
...
def test_validate_card_ok_and_badlink():
    assert validate_card(CARD_OK, {"wave", "gadgets/replenishment"}) == []
    errs = validate_card(CARD_BADLINK, {"wave", "gadgets/replenishment"})
...
def test_validate_card_missing_key():
    ...
    errs = validate_card(card_missing_type, {"wave", "gadgets/replenishment"})
...
def test_promote_moves_only_approved(tmp_path):
    ...
    drafts.mkdir()
    concepts.mkdir()
    (drafts / "wave.md").write_text(CARD_OK)  # related: [gadgets/replenishment]
    (drafts / "replenishment.md").write_text(CARD_DRAFT)  # sibling draft supplies prospective id
    promoted, invalid = promote(drafts, concepts, "gadgets")
    assert promoted == ["wave.md"]
    assert (concepts / "gadgets" / "wave.md").exists()
...
def test_promote_writes_into_product_subfolder(tmp_path):
    ...
    promoted, invalid = promote(drafts, concepts, "sprockets")
    assert promoted == ["wave.md"]
    assert (concepts / "sprockets" / "wave.md").exists()
...
def test_promote_known_ids_include_prospective_product_path(tmp_path):
    ...
    (drafts / "replenishment.md").write_text(
        ...
        .replace("related: [gadgets/replenishment]", "related: []"))
    promoted, invalid = promote(drafts, concepts, "gadgets")
    assert set(promoted) == {"wave.md", "replenishment.md"}
    assert invalid == {}
    assert (concepts / "gadgets" / "wave.md").exists()
    assert (concepts / "gadgets" / "replenishment.md").exists()
